...
def bfs(capacity_matrix, flow_matrix, source, sink, parent):
    ...
    while queue:
        ...
        for v in range(num_nodes):
            ...
            # 1. Сусід v ще не відвіданий.
            # 2. Залишкова пропускна здатність на ребрі (u, v) > 0.
            #    (capacity[u][v] - flow[u][v] = залишкова пропускна здатність)
...
# Створення словника для відображення імен вузлів на їх числові індекси (0-20).
nodes = {
    'S': 0, 'T': 20, 
    ...
    'Магазин 11': 17, 'Магазин 12': 18, 'Магазин 13': 19, 'Магазин 14': 20 # M14 та T мають однаковий індекс 20.
}
# Загальна кількість вузлів.
num_nodes = 21 
# Велике число, що позначає необмежену пропускну здатність.
INF = 1000 
# Ініціалізація матриці пропускної здатності розміром 21x21 нулями.
...
pos[nodes['S']] = (0, 0)
pos[nodes['T']] = (10, 0)
pos[nodes['Термінал 1']] = (2, 1.5)
pos[nodes['Термінал 2']] = (2, -1.5)
pos[nodes['Склад 1']] = (5, 2.5)
pos[nodes['Склад 2']] = (5, 0.5)
pos[nodes['Склад 3']] = (5, -0.5)
pos[nodes['Склад 4']] = (5, -2.5)

# Координати Y для магазинів.
shop_y_coords = [3.5, 3.0, 2.5, 1.5, 1.0, 0.5, -0.5, -1.0, -1.5, -2.5, -3.0, -3.5, -4.0, -4.5]
# Встановлення позицій для всіх магазинів.
for i, name in enumerate(shops):
    pos[nodes[name]] = (8, shop_y_coords[i] - 1.0) 

# --- 4. Візуалізація Графа (Блок закоментовано) ---

# Блок візуалізації закоментовано, оскільки виконання графічних команд (plt.show())
# не підтримується у текстовому середовищі, але демонструє повну логіку.
...
for w in warehouses:
    ...
    for t in terminals:
        t_idx = nodes[t]
        warehouse_inflow[w] += flow_matrix[t_idx][w_idx] # Сумарний потік Термінали -> Склад.

# Логіка Реконструкції Потоку: Крок 2 (Частка Термінала)
term_to_warehouse_share = {t: {} for t in terminals}
for t in terminals:
    for w in warehouses:
        inflow = flow_matrix[nodes[t]][nodes[w]]
        total_inflow = warehouse_inflow[w]
        # Обчислюємо частку, яку Термінал t вніс у загальний потік Складу w.
...
for w in warehouses:
    for s in shops:
        warehouse_to_shop_flow = flow_matrix[nodes[w]][nodes[s]] # Потік Склад -> Магазин.
        for t in terminals:
...
# --- 5. Вивід Звіту та Аналіз (ФІНАЛЬНИЙ ЧИСТИЙ БЛОК) ---
...
print(f"Це вузьке місце критично лімітує загальний потік мережі до {max_flow} одиниць (за Теоремою Max-Flow Min-Cut).")
print(f"Індивідуально найменший канал: Склад 4 -> Магазин 13 ({capacity_matrix[nodes['Склад 4']][nodes['Магазин 13']]} одиниць).")
...
# # --- 5. Вивід Звіту та Аналіз (ОНОВЛЕНО ДЛЯ ЧИСТОГО ТЕРМІНАЛЬНОГО ВИВОДУ) ---

# # Виводимо максимальний потік у форматі чистого тексту.
# print(f"✅ Максимальний потік у мережі: {max_flow} одиниць")
# print("----------------------------------------------------------------------")
# print("📊 Звіт: Аналіз Логістичної Мережі Максимального Потоку")
# print("----------------------------------------------------------------------")
...
# # 5.1. Пояснення моделі та обчислення
# print("\n=== Пояснення Моделі та Результати Обчислень ===")
# # Замінюємо LaTeX-форматування та жирний шрифт на чистий текст.
# print("Побудована модель графа коректно відображає логістичну мережу з 2 Терміналами, 4 Складами та 14 Магазинами, об'єднаними Суперджерелом (S) та Суперстоком (T).")
# print(f"Розрахунок алгоритму Едмондса-Карпа показав, що максимальний обсяг постачання становить {max_flow} одиниць.")
# print("Це значення підтверджує Теорему про мінімальний розріз-максимальний потік.")
# print("Таким чином, досягнуто оптимального потоку.")
...
# # 5.2. Таблиця Потоків
# print("\n=== Таблиця: Фактичний Потік Термінал -> Магазин ===")
# # Використовуємо звичайні роздільники для імітації таблиці в терміналі.
# header = f"{'Термінал':<12} | {'Магазин':<10} | {'Фактичний Потік (одиниць)':<25}"
# print(header)
# print("-" * len(header))
# for item in final_report_table:
#     # Вирівнюємо стовпці за шириною заголовка.
#     print(f"{item['Термінал']:<12} | {item['Магазин']:<10} | {item['Фактичний Потік']:<25}")
# print("-" * 70)
...
# # # 5.1. Пояснення моделі та обчислення
# # print("## Пояснення Моделі та Результати Обчислень")
# # print("Побудована модель графа коректно відображає логістичну мережу з 2 Терміналами, 4 Складами та 14 Магазинами, об'єднаними $\text{Суперджерелом (S)}$ та $\text{Суперстоком (T)}$.")
# # print(f"Розрахунок алгоритму Едмондса-Карпа показав, що максимальний обсяг постачання становить **{max_flow} одиниць**.")
# # print("Це значення підтверджує **Теорему про мінімальний розріз-максимальний потік**.")
# # print("Таким чином, досягнуто **оптимального потоку**.")
# # print("-" * 70)
...
# # # 5.2. Таблиця Потоків
# # print("## Таблиця: Фактичний Потік Термінал → Магазин")
...
# # # 5.3. Відповіді на Запитання
# # flow_term1 = sum(item['Фактичний Потік'] for item in final_report_table if item['Термінал'] == 'Термінал 1')
# # flow_term2 = sum(item['Фактичний Потік'] for item in final_report_table if item['Термінал'] == 'Термінал 2')

# # # print("## Аналіз Отриманих Результатів та Відповіді на Запитання")
...
# # # print(f"Найменше товарів отримав **Магазин 13** (**{min_shop_flow['Потік до Магазину']}** одиниць).")
# # # print(f"**Так**, його постачання можна збільшити, збільшивши пропускну здатність маршруту **Склад 4 → Магазин 13**.")
...
# # # Логіка виведення сумарних потоків (ВИПРАВЛЕНО, щоб не було повторів/суперечностей)
# # if flow_term1 == flow_term2:
# #     print(f"Термінал 1 постачає {flow_term1} одиниць. Термінал 2 постачає {flow_term2} одиниць.")
# #     print("Обидва термінали забезпечують однаковий потік.")
# # elif flow_term1 > flow_term2:
...
# # min_shop_flow = min(shop_flows, key=lambda x: x['Потік до Магазину'])
# # print("\n3. Які магазини отримали найменше товарів і чи можна збільшити їх постачання?")
# # print(f"Найменше товарів отримав Магазин {min_shop_flow['Магазин'].split(' ')[1]} ({min_shop_flow['Потік до Магазину']} одиниць).")
# # print(f"Найбільше товарів отримав Магазин {max_shop_flow['Магазин'].split(' ')[1]} ({max_shop_flow['Потік до Магазину']} одиниць).")
# # print(f"Так, постачання Магазину {min_shop_flow['Магазин'].split(' ')[1]} можна збільшити, збільшивши пропускну здатність маршруту Склад 4 -> Магазин 13.")
...
# # --- 5.3. ОНОВЛЕНИЙ ВИВІД АНАЛІЗУ (ВЕСЬ БЛОК) ---
# # 5.3. Відповіді на Запитання

# # Обчислення сумарних потоків Терміналів
# flow_term1 = sum(item['Фактичний Потік'] for item in final_report_table if item['Термінал'] == 'Термінал 1')
# flow_term2 = sum(item['Фактичний Потік'] for item in final_report_table if item['Термінал'] == 'Термінал 2')
...
# # 3. Які магазини отримали найменше товарів і чи можна збільшити їх постачання?
# # Використовуємо оновлену логіку з пошуком всіх мінімальних магазинів
# min_flow_value = min(item['Потік до Магазину'] for item in shop_flows)
# min_flow_shops = [item['Магазин'] for item in shop_flows if item['Потік до Магазину'] == min_flow_value]
# min_flow_shops_names = [name.split(' ')[1] for name in min_flow_shops] 
